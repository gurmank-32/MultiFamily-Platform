...
class QASystem:
    ...
    def answer_question(self, question: str, city: str = None) -> Dict:
        ...
        # Handle hard-coded demo questions with specific sources
        question_lower = question.lower()
        
        # Hard-coded answer for "What is ESA?"
        if ("what is esa" in question_lower or "what is an esa" in question_lower or 
            (question_lower.strip() == "what is esa?" or question_lower.strip() == "what is esa")):
            return {
                ...
                "sources": [
                    {
                        "source": "Texas ESA",
                        "url": "https://guides.sll.texas.gov/animal-law/service-animals",
                        "category": "ESA"
                    },
                    {
        ...
        # Hard-coded answer for "What is rent control in Dallas, TX?"
        if ("what is rent control" in question_lower and "dallas" in question_lower) or \
           ("rent control" in question_lower and "dallas" in question_lower and ("what is" in question_lower or "tell me" in question_lower)):
            return {
                "answer": "Rent control in Dallas, Texas refers to laws that limit how much landlords can increase rent. Dallas has enacted a new rent control policy that prohibits landlords from raising rent by more than $250 above the tenant's existing monthly rent. This cap applies regardless of property improvements, upgrades, or neighborhood characteristics. For leasing managers, this means every rent increase must be reviewed carefully before issuing renewal offers or updated lease agreements.",
                "sources": [
                    {
                        "source": "Texas State Law Library - Rent Control",
                        "url": "https://guides.sll.texas.gov/landlord-tenant-law/rent?utm_source=chatgpt.com",
        ...
        # Hard-coded answer for "What is new law in Dallas?"
        if ("what is new law" in question_lower and "dallas" in question_lower) or \
           ("new law" in question_lower and "dallas" in question_lower and "what is" in question_lower) or \
           ("what's new" in question_lower and "dallas" in question_lower):
            return {
                "answer": "A newly announced policy in Dallas, Texas claims that landlords are now prohibited from raising rent by more than $250 above the tenant's existing monthly rent. For example, if a tenant currently pays $1,000 per month, the maximum legal rent after an increase would be $1,250. If the rent today is $2,000, the legal maximum would be $2,250. Under this rule, it would not matter whether the unit is remodeled, upgraded, or located in a premium neighborhood — the increase still cannot exceed a $250 gap from the prior lease.\n\nFor leasing managers, this alleged requirement would mean that every rent increase must be reviewed carefully before issuing renewal offers or updated lease agreements. Standard automated percentage-increase clauses (like 5–10% annual increases) would need to be paused or rewritten to ensure they never exceed a $250 difference. Leasing teams would be responsible for ensuring internal rent systems, resident notices, renewal documentation, and lease contract language all comply with this new adjustment cap.\n\nOperationally, property managers would need to maintain detailed rent-change records for each tenant to prove that no unlawful increase occurred. Staff would also need to review budgeting and revenue planning, since limiting increases might reduce projected income for the property. If leasing managers fail to monitor this properly, tenants could dispute their leases or file compliance complaints — leading to possible legal and financial consequences for the landlord.",
                "sources": [
                    {
                        "source": "Dallas Rent Control 2025 (DEMO)",
                        "url": "test_rent_control_dallas_demo.html",
